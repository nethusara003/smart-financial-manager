...
async function collectMetrics() {
  try {
    ...
    try {
      ...
      metrics.userId = registerData.user.id || registerData.user._id;
      metrics.jwt = registerData.token;
    ...
    try {
      const forecastResponse = await axios.get(`${BaseURL}/forecasting/expenses`, authConfig);
